...
class RandomNumberGenerator {
    ...
    /*
    static func generateRandomPositions(rangeX: (Int, Int), rangeY: (Int, Int), imgSize: Int, sampleSize: Int) ->[(Int, Int)] {
    ...
    static func generateRandomPositions(rangeX: Int, rangeY: Int, imgSize: (CGFloat, CGFloat), sampleSize: Int) ->[(Int, Int)] {
        var retPositions = [(Int, Int)]()
        
        for _ in 0..<sampleSize {
            let randomX = Int(arc4random_uniform(UInt32(rangeX)))
            let randomY = Int(arc4random_uniform(UInt32(rangeY)))
            retPositions.append((Int(CGFloat(randomX) * imgSize.0),
                                 Int(CGFloat(randomY) * imgSize.1)))
        }
        return retPositions
    }
}
